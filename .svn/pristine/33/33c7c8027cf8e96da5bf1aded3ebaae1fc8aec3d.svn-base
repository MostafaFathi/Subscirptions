...
$(function() {
    var base_url = 'http://' + window.location.host+'/';
    $(document).on('submit', '#student_submit_form', function() {
        ...
        if(add_button)
            navigate_to+='students/add';
        else if(renew_button)
            navigate_to+='students/'+student_id+'/renew';
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                validation(success,'student_name',true,"#student_submit_form");
                if(add_button)
                    notificationMessage('رسالة نجاح','تم إضافة طالب جديد بنجاح!','bg-success') ;
                else if(renew_button)
                    notificationMessage('رسالة نجاح','تم تجديد اشتراك الطالب بنجاح!','bg-success') ;
                ...
                validation(errors,'student_name',false,"#student_submit_form");
    ...
    $(document).on('submit', '.student_update_form', function() {
        var this_form = $(this);
        var submit = $(this).find("button[type='submit'] > .loader");
        ...
        $.ajax({
            type: 'post',
            dataType: "json",
            url: base_url+'students/'+student_id+'/update',
            data: new FormData(this),
            cache: "false",
            contentType: false,
            processData: false,
            success: function(data) {
                this_form.find('.text_student_name').text(data['student'].student_name);
                this_form.find('.text_student_address').text(data['student'].student_address);
                this_form.find('.text_student_father_work').text(data['student'].student_father_work);
                this_form.find('.text_student_phone').text(data['student'].student_phone);
                this_form.find('.text_student_phone2').text(data['student'].student_phone2);
                this_form.find('.text_student_age').text(data['student'].student_age);
                this_form.find('.text_student_school').text(data['student'].student_school);
                this_form.find('.text_student_reg_date').text(data['student'].student_reg_date);
                if(data['student'].student_gender == 'f')
                    this_form.find('.text_student_gender').text('أنثى');
                else
                    this_form.find('.text_student_gender').text('ذكر');

                var level_select_box = this_form.find('#level_id');
                var student_level =  level_select_box.find('option[value='+level_select_box.val()+']').text();
                this_form.find('.text_level_name').text(student_level);

                var branch_select_box = this_form.find('#branch_id');
                var student_branch =  branch_select_box.find('option[value='+branch_select_box.val()+']').text();
                this_form.find('.text_branch_name').text(student_branch);

                this_form.parent().parent().parent().prev().find('.td-level-name').text(student_level);
                this_form.parent().parent().parent().prev().find('.td-branch-name').text(student_branch);
                this_form.parent().parent().parent().prev().find('.td-reg-date').text(data['student'].student_reg_date);
                var success = data.responseJSON;
                validation(success,'student_name',true,'.student_update_form');

                notificationMessage('رسالة نجاح','تم تعديل الطالب بنجاح!','bg-success') ;
                loader(submit,false);// hide loader and un-disable button
                ...
                var errors = data.responseJSON;
                validation(errors,'student_name',false,'.student_update_form');

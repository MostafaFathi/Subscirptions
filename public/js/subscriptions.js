...
$(function() {
    ...
    $(document).on('submit', '#subscription_submit_form', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                validation(success,'product_name',true,"#subscription_submit_form");
                if(add_button)
                    notificationMessage('رسالة نجاح','تم إضافة إشتراك جديد بنجاح!','bg-success') ;
                ...
                validation(errors,'name',false,"#subscription_submit_form");
            }
    ...
    $(document).on('submit', '.subscription_update_form', function() {
        var this_form = $(this);
        var submit = $(this).find("button[type='submit'] > .loader");
        var subscriptions_id = $(this).find(".subscriptions_id").val();
        loader(submit,true);// show loader and disable button
        $.ajax({
            type: 'post',
            dataType: "json",
            url: base_url+'subscription/'+subscriptions_id+'/update',
            data: new FormData(this),
            cache: "false",
            contentType: false,
            processData: false,
            success: function(data) {
                ...
                var interval_select_box = this_form.find('#interval');
                var interval =  interval_select_box.find('option[value='+interval_select_box.val()+']').text();

                this_form.parent().parent().parent().prev().find('.name').text(data['subscription'].name);
                this_form.parent().parent().parent().prev().find('.address').text(data['subscription'].address);
                this_form.parent().parent().parent().prev().find('.phone').text(data['subscription'].phone);
                this_form.parent().parent().parent().prev().find('.hints').text(data['subscription'].hints);
                this_form.parent().parent().parent().prev().find('.health').text(data['subscription'].health);
                this_form.parent().parent().parent().prev().find('.payment').text(data['subscription'].payment);
                this_form.parent().parent().parent().prev().find('.end_at').text(data['subscription'].end_at);
                this_form.parent().parent().parent().prev().find('.interval').text(interval);

                var success = data.responseJSON;
                validation(success,'name',true,'.subscription_update_form');

                notificationMessage('رسالة نجاح','تم تعديل الاشتراك بنجاح!','bg-success') ;
                loader(submit,false);// hide loader and un-disable button
                ...
                var errors = data.responseJSON;
                validation(errors,'name',false,'.subscription_update_form');
            ...
            function(isConfirm){
                if (isConfirm) {
                    ...
                    $.ajax({
                        type: 'get',
                        dataType: "json",
                        url: base_url+ 'subscription/'+subscriptions_id+'/delete',
                        data: "",
                        cache: "false",
                        success: function(data) {
